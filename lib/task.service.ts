...
export class TaskService {
    ...
    try {
      ...
      await Promise.all(
        users.map(async (user) => {
          const {
            data: { data: tweets },
          } = await this.x.v2.userTimeline(user.profileId, {
            max_results: 50,
          ...
          const existingTweetMap = new Map(
            existingTweets.map((tweet) => [tweet.postId, tweet]),
          );

          const tweetPromises = tweets.map(
            async ({
              id,
              author_id,
              text,
              ...
              if (
                tweetCreatedAt < campaignStartDate ||
                tweetCreatedAt > campaignExpiryDate
              ) {
                return;
              }

              const tags = settings.tags.map((tag) => tag.toLowerCase().trim());
              const containsTag = tags.some((tag) =>
                text.toLowerCase().trim().includes(tag),
              );

              if (containsTag) {
                let referenced = false;
                if (author_id !== settings.profileId && referenced_tweets) {
                  referenced = await this.checkReferencedTweets(
                    referenced_tweets,
                    settings.profileId,
                  );
                }
  ...
  private async checkReferencedTweets(
    ...
    for (const chunk of tweetChunks) {
      ...
      for (const tweet of tweets) {
        ...
        if (tweet.referenced_tweets) {
          const nestedReferenced = await this.checkReferencedTweets(
    ...
    try {
      ...
      for (const chunk of tweetChunks) {
        ...
        const updatePromises = tweets.map(async (tweet) => {
          if (tweet.referenced_tweets) {
            const referenced = await this.checkReferencedTweets(
              tweet.referenced_tweets,
              settings.profileId,
            );
            if (referenced) {
      ...
      throw new HttpException(
        'Process existing tweets task failed',
